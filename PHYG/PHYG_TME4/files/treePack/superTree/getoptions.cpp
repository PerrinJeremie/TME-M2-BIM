       ...
       int *ret_optind, char **ret_optname, char **ret_optarg)
{
  ...
  if (optptr == NULL && strncmp(argv[optind], "--", 2) == 0)
	...
	if (opt[i].single == false &&
      ...
      if (nmatch > 1)
      {
      	std::cerr << "Option \"" << argv[optind] << "\" is ambiguous; please be more specific."
        	<< std::endl << usage << std::endl;
        exit (0);
		}
      if (nmatch == 0)
      {
      	std::cerr << "No such option \"" << argv[optind] << "\"."
        	<< std::endl << usage << std::endl;
        exit(0);
  	}
      ...
      if (opt[opti].argtype != ARG_NONE) 
	{
	  if (optind+1 >= argc)
      {
      	std::cerr << "Option " << opt[opti].name << "\" requires an argument."
        	<< std::endl << usage << std::endl;
        exit (0);
		}
	...
	if (opt[i].single == true && *optptr == opt[i].name[1])
      ...
      if (opti == -1)
      {
      	std::cerr << "No such option \"" << *optptr << "\"."
        	<< std::endl << usage << std::endl;
        exit(0);
  	}
      *ret_optname = opt[opti].name;
  ...
  if (opt[opti].argtype != ARG_NONE) 
    {
      ...
      {
      	std::cerr << "Option " << opt[opti].name << "requires an integer argument."
        	<< std::endl << usage << std::endl;
        exit(0);
  	}
      else if (opt[opti].argtype == ARG_FLOAT && ! IsReal(*ret_optarg))
      {
      	std::cerr << "Option " << opt[opti].name << "requires a numerical argument."
        	<< std::endl << usage << std::endl;
        exit(0);
  	}
      else if (opt[opti].argtype == ARG_CHAR && strlen(*ret_optarg) != 1)
      {
      	std::cerr << "Option " << opt[opti].name << "requires single-character argument."
        	<< std::endl << usage << std::endl;
        exit(0);
  	}
